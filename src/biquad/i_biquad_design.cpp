...
void IBiquad::ProcessBlock(DspBlock *process_buf)
{
    for (size_t i = 0; i < process_buf->block_size; ++i) {
        float in = process_buf->dsp_buffer[i];
...
void IBiquad::ProcessBlockFixed(DspBlock *process_buf)
{
    for (size_t i = 0; i < process_buf->block_size; ++i) {
        tFixed in = process_buf->dsp_buffer[i];
